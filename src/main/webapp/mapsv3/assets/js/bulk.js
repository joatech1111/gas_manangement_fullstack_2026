...
var bulkStartLocationMarkers = [];
var bulkStartTimeMarkers = [];
...
function showRoute(bShow) {
	for (var i in bulkTrackingLines) {
		for (var j in bulkTrackingLines[i]) {
			bulkTrackingLines[i][j].setVisible(bShow);
		}
	}
	
	for (var i in bulkStartLocationMarkers) {
		bulkStartLocationMarkers[i].setVisible(bShow);
	}	
	
	for (var i in bulkStartTimeMarkers) {
		bulkStartTimeMarkers[i].setVisible(bShow);
	}	
...
function clearRoute() {
	for (var i in bulkTrackingLines) {
		for (var j in bulkTrackingLines[i]) {
			bulkTrackingLines[i][j].setMap(null);
		}
	}
	...
	for (var i in bulkStartLocationMarkers) {
		bulkStartLocationMarkers[i].setVisible(null);
	}	
	
	bulkStartLocationMarkers = [];

	for (var i in bulkStartTimeMarkers) {
		bulkStartTimeMarkers[i].setVisible(null);
	}	
	
	bulkStartTimeMarkers = [];
...
function updateRoute(bulkGpsLines, useBounds) {
	...
	for (var i in bulkGpsLines) {
		...
		var curCarCode = oBulkGpsLine.carCode;		
		if (curCarCode == prevCarCode) {
			...
			if (prevLatLng) {
				routePathArray[curCarCode].push([prevLatLng, curLatLng]);
			}
			...
			routePathArray[curCarCode] = [];
			
			prevCarCode = curCarCode;
			
			var nCurLat = Number(oBulkGpsLine.gpsY);
			var nCurLng = Number(oBulkGpsLine.gpsX);
			var curLatLng = new naver.maps.LatLng([nCurLat, nCurLng]);
			
			bulkStartLocationMarkers[i] = addBulkStartLocationMarker(curLatLng, oBulkGpsLine, visibleRoute);
			
			bulkStartTimeMarkers[i] = addBulkStartTimeMarker(curLatLng, oBulkGpsLine, visibleRoute);
	...
	for (var i in routePathArray) {
		bulkTrackingLines[i] = [];
	    for (var j in routePathArray[i]) {
	    	bulkTrackingLines[i][j] = new naver.maps.Polyline({
		        path: routePathArray[i][j],
		        map: map,
		        strokeColor: '#0000FF',
...
function addBulkLocationMarker(latlng, data, visible) {	
	...
	var carName = data.carName;
        var receiveTime = data.receiveTime.substr(0,2) + ":" + data.receiveTime.substr(2,2) + ":" + data.receiveTime.substr(4,2) ;
		...
		icon: {
			content: [
			          '<div class="bulk_info_cell bulk_map">',
			          	...
			          	'<div class="bulk_time">' + receiveTime+ '</div>',
			          '</div>'
			          ].join(''),
...
function addBulkStartLocationMarker(latlng, data, visible){
	var width = 20;
	var height = 20;
...
function addBulkStartTimeMarker(latlng, data, visible){
	var width = 178;
	var height = 48;
	
	var chargeTime = getFormattedTime(data.receiveTime);
	var carName = data.carName;
	...
	var marker = new naver.maps.Marker({
		...
		icon: {
			content: [
			          '<div class="start_time_cell">',
			          	'<div class="customer_name">' + carName + '</div>',
			          	'<div class="route_time">' + chargeTime + '</div>',
			          '</div>'
			          ].join(''),
			size: new naver.maps.Size(width, height),
