...
function getMultiAppUser() {
	...
	$.ajax({
		url: gasmaxeyeWebappPath + "app_user_multi_check_ajx.jsp",
		type: "post",
		...
		success: function (xml) {
			...
			if (totalRowCount > 1) {
				var remember_co = window.localStorage["remember_gasmaxeye_co"];
...
function authCheck() {
	...
	$.ajax({
		url: gasmaxWebappPath + "auth_check_ajx.jsp",
		type: "post",
		...
		success: function (xml) {
			...
			if (result == "Y") { //로그인 성공!!
				...
				var html = '<table style="width: 100% ; ">'
					...
					+ '	</tr>'
					+ '</table>';
				$("#divLoginFailMessage").html(html).show().trigger("create");
			}
			$("#loginMessage").html("").trigger("create");
		}
	});
...
function choiceCustomer(areaCode, customerCode) {
	...
	// START EDIT: 롱클릭 메뉴에서 호출된 경우, 이전페이지 정보를 롱클릭 메뉴을 호출한 페이지로 설정
	if ($("#hdnCurrentPage").attr("value") == "dialogCustomerBizMenu") {
		previousPage = $("#hdnCallPageDiaglogCustomerBizMenu").attr("value");
	}
	// END EDIT
...
function injectionAppUserInsert(tagId) {
	...
	$.ajax({
		...
		type: "post",
		data: "macNumber=" + $("#hdnUuid").attr("value"),
		dataType: "html",
		timeout: 60000,
		error: function (result) {
			if (result.status == 0) {
...
function injectionCustomerSaleWeightInsert(tagId) {
	$("#" + tagId).html(getResultMessage("잠시만 기다려주세요.", true)).trigger("create");
	$.ajax({
		url: gasmaxWebappPath + "customer_sale_weight_insert_batch.jsp",
		type: "post",
		dataType: "html",
		timeout: 60000,
		error: function (result) {
...
function clickSaveAndInsertCustomerCollectInsert() {
	...
	var returnCode = saveCustomerCollectInsert();
	if (returnCode == "S") { showPageCustomerSearch(); }
}
...
function injectionCustomerSaftyCheckInsert(tagId) {
	...
	$.ajax({
		url: gasmaxWebappPath + "customer_safty_check_insert_rev3.jsp",
		type: "post",
		dataType: "html",
		timeout: 60000,
		async: false,
...
function injectionCustomerSaftyCheckTankInsert(tagId) {
	...
	$.ajax({
		url: gasmaxWebappPath + "customer_safty_check_tank_insert.jsp",
		type: "post",
		dataType: "html",
		timeout: 60000,
		async: false,
...
function closePipeLengthOptions() {
	$.mobile.changePage("#" + $("#hdnCallPageDialogPipeLengthOptions").attr("value"), { changeHash: false, reverse: true });
