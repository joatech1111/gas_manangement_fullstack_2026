...
%><%
	try{
		...
		if (signatureImage.length() > 0) signatureYn = "Y";
		...
		if (appUser != null){
			...
			// 신규등록이라면 Sno 새로 발급 받아 처리
			sequenceNumber = BizCustomerSaftyCheck.getInstance().selectTankSno(serverIp, catalogName, areaCode, customerCode);
			if (sequenceNumber == null) {
				sequenceNumber = "T001";
			}
			CustomerSaftyCheckTank resultCustomerSaftyCheckTank = BizCustomerSaftyCheck.getInstance().insertCustomerSaftyCheckTank(serverIp, catalogName, areaCode, customerCode, sequenceNumber, scheduledCheckDate, employeeCode, employeeName, tankCapacity1, tankCapacity2, acceptable1, acceptable1Comment, acceptable2, acceptable2Comment, acceptable3, acceptable3Comment, acceptable4, acceptable4Comment, acceptable5, acceptable5Comment, acceptable6, acceptable6Comment, acceptable7, acceptable7Comment, acceptable8, acceptable8Comment, acceptable9, acceptable9Comment, acceptable10Content, acceptable10, acceptable10Comment, acceptable11Content, acceptable11, acceptable11Comment, acceptable12Content, acceptable12, acceptable12Comment, employeeComment1, employeeComment2, customerName, signatureYn, userId, modifyDate);

			// 서명 저장(서명이 등록되어 있지 않아도 생성)
			CustomerSaftyCheckSignature resultCustomerSaftyCheckSignature = BizCustomerSaftyCheck.getInstance().insertCustomerSaftyCheckSignature(serverIp, catalogName, areaCode, StringUtil.convertCustomerCodeToTankFormat(customerCode), sequenceNumber, scheduledCheckDate, signatureImage, userId, modifyDate);
